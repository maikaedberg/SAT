module Solver.TwoWatch (solution) where
...
update_altwatch :: Lit -> [Int] -> [Cls] -> [(Lit, [Int])] -> ([(Lit, [Int])], [Int])
update_altwatch _ [] _ alt_watch = (alt_watch, [])
update_altwatch rid_of (n: rest) curr_clauses alt_watch = (alt_watch''', new_ucs)
  where
    alt_watch' = replace (rid_of, []) alt_watch
    new_watch = find_new_watch rid_of n curr_clauses alt_watch
    alt_watch'' = append n new_watch alt_watch'
    (alt_watch''', prev_ucs) = update_altwatch rid_of rest curr_clauses alt_watch''
    new_ucs = 
      case new_watch of
        [] -> n : prev_ucs 
        otherwise -> prev_ucs

solve :: [Int] -> [Int] -> [Cls] -> [(Lit, [Int])] -> Maybe Subst
solve _ _ [] _ = Just []
solve possibilities more_possibilities curr_clauses alt_watch =
  if (elem [] (map literals curr_clauses))
    ...
    where
      ...
      (cond, found_uc) = 
        case more_possibilities of
          [] -> 
            case find_uc possibilities alt_watch of
              Nothing -> (head (literals (head curr_clauses)), False)
              Just sub -> (head (literals (find_cls sub curr_clauses)), True)
          otherwise ->
            (head (literals (find_cls (head more_possibilities) curr_clauses)), True)

      to_update_x = 
        case lookup (negLit cond) alt_watch of
          Nothing -> []
          Just replace_clss -> replace_clss
      (new_watch_x, possible_ucs_x) = update_altwatch (negLit cond) to_update_x curr_clauses alt_watch

      to_update_negx = 
        case lookup (cond) alt_watch of
          Nothing -> []
          Just replace_clss -> replace_clss
      (new_watch_negx, possible_ucs_negx)= update_altwatch (cond) to_update_negx curr_clauses alt_watch

      cond_x    = condition cond curr_clauses
      cond_negx = condition (negLit cond) curr_clauses
      rho_x     = solve to_update_x (more_possibilities ++ possible_ucs_x) cond_x new_watch_x
      rho_negx  =
        case found_uc of
          True -> Nothing
          False -> 
            (solve to_update_negx (more_possibilities ++ possible_ucs_negx) cond_negx new_watch_negx)

solution :: CNF -> Maybe Subst
solution cnf =
  case solve possibilities [] curr_clauses alt_watch of
    Nothing -> Nothing
  ...
  where
    ...
    possibilities = map index curr_clauses
